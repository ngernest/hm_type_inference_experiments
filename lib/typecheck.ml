...
let rec apply_subst (subst : sub) t =
  match t with
 ...
 * Returns a substitution that most weakly unifies the constraint (t1 = t2) for types t1, t2.
 *)
let rec unify t1 t2 : sub = failwith "not implemented"

(* lookup a variable in the context *)
(* raises TypeError if variable is not found *)
 ...
 *  - s is a substitution containing all the constraints that must be satisfied for e to be of type t.
 *)
let rec infer (ctx : context) e : typ * sub = failwith "not implemented"
